...
def save_pruned_model(model, tokenizer, save_directory):
    ...
    custom_config.save_pretrained(save_directory)
    ...
    # Save additional metadata about layer sizes
    layer_metadata = {}
    for i, layer in enumerate(model.model.layers):
        layer_metadata[f"layer_{i}"] = {
            "self_attn.q_proj.weight": layer.self_attn.q_proj.weight.shape,
            "self_attn.k_proj.weight": layer.self_attn.k_proj.weight.shape,
            "self_attn.v_proj.weight": layer.self_attn.v_proj.weight.shape,
            "self_attn.o_proj.weight": layer.self_attn.o_proj.weight.shape,
            "mlp.gate_proj.weight": layer.mlp.gate_proj.weight.shape,
            "mlp.up_proj.weight": layer.mlp.up_proj.weight.shape,
            "mlp.down_proj.weight": layer.mlp.down_proj.weight.shape,
        }

    with open(f"{save_directory}/pruned_metadata.json", "w") as f:
